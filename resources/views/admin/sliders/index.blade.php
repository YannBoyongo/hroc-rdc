<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-deep_twilight-500">Liste des sliders</h2>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($slider->image)
                                    <img src="{{ Storage::url($slider->image) }}" alt="{{ $slider->title }}" class="h-16 w-24 object-cover rounded">
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-deep_twilight-500">{{ $slider->title }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.sliders.destroy', $slider) }}" 
                                          method="POST" 
                                          class="inline"
                                          onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce slider ?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-4 text-center text-gray-500">
                                Aucun slider trouvé. <a href="{{ route('admin.sliders.create') }}" class="text-blue_green-600 hover:underline">Créer le premier</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
